...
__device__ float atomic_max(float *addr, float val) {
    int *old_addr = reinterpret_cast<int*>(addr);
    int old_val = *old_addr;
    int expected;
    do {
        expected = old_val;
        old_val = atomicCAS(old_addr, expected, __float_as_int(fmaxf(__int_as_float(old_val), val)));
    } while(expected != old_val);
    return old_val;
}

// dim3 block(BLOCK_SIZE)
// dim3 grid(batch * blocks_per_row)
// sum_reduce_atomic<<<grid, block>>>
__global__ void sum_reduce_atomic(float *x, float *y, const int L, const int blocks_per_row) {
