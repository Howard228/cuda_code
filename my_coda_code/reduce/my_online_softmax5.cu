...
// cuda_online_softmax_naive<<<CEIL(batch, BLOCK_SIZE), BLOCK_SIZE>>>
__global__ void cuda_online_softmax_naive(const float *input, float *output, const int L) {
    int tid = threadIdx.x;
...
// cuda_online_softmax_naive<<<batch, BLOCK_SIZE>>>
__global__ void cuda_online_softmax(const float *input, float *output, const int L) {
    ...
    for (int i = tid; i < L; i += stride) {
        ...
        local_sum = local_sum * expf(old_max - local_max) + expf(input[i] - local_max);
    }
